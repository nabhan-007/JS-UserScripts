...
(function () {
  ...
  function clickReadMore(container) {
    container
      .querySelectorAll('button, [role="button"], span, div, p, a')
      .forEach((el) => {
        if (
  ...
  function scrollToLast() {
    ...
    // If all topics share the same state (all expanded or all collapsed),
    // there's no meaningful "last read" position — skip scroll.
    const expanded = containers.filter(isExpanded).length;
    if (expanded === 0 || expanded === containers.length) return;
